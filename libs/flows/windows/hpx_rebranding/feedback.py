...
class Feedback(HPXRebrandingFlow):
    ...
    def verify_edit_feedback_screen_opens(self):
        """
        Verify that the edit feedback screen is displayed with all key elements.
        
        :return: True if all elements are successfully found, otherwise raises exception
        """
        self.driver.wait_for_object("hpx_feedback_page_title")
        self.driver.wait_for_object("tell_your_experience_title")
        self.driver.swipe("edit_feedback")
        self.driver.wait_for_object("edit_feedback")
        self.driver.swipe("send_feedback_submit_btn")
        self.driver.wait_for_object("send_feedback_submit_btn")
        return True
    ...
    def input_tell_your_experience_text(self, text: str):
        ...
        self.driver.swipe("edit_feedback")
        self.driver.wait_for_object("edit_feedback")
        self.driver.click("edit_feedback")
        element = self.driver.wait_for_object("edit_feedback")
        existing_value = element.get_attribute("Value.Value")
        if existing_value:
            element.clear()
        self.driver.send_keys("edit_feedback", text)

    def assert_tell_your_experience_text_is_truncated(self, expected_max_length: int):
        """
        Verify that the text entered in the 'tell your experience' field is truncated to the maximum character limit.
        
        :param expected_max_length: The expected maximum character length (e.g., 1000)
        :raises AssertionError: If text is not properly truncated
        """
        self.driver.swipe("edit_feedback")
        self.driver.wait_for_object("edit_feedback")
        entered_text = self.get_entered_text()
        text_length = len(entered_text) if entered_text else 0
        
        assert text_length <= expected_max_length, f"Text length {text_length} exceeds maximum allowed length {expected_max_length}"
        ...
        if "/" in char_count_display:
            ...
            assert current_count == text_length, f"Character count display {current_count} does not match actual text length {text_length}"
            assert current_count <= expected_max_length, f"Character count {current_count} exceeds expected maximum {expected_max_length}"
            assert max_count == expected_max_length, f"Maximum count display {max_count} does not match expected {expected_max_length}"
        else:
